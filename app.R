...
mods <- c("AC", "CN", "EC", "GF", "GI", "MI", "MP", "MR", "EN")
  ...
  output$downloadMap <- downloadHandler(
    ...
    content = function(file) {
      ...
      if(input$type!=3){
        ...
        png(file, width = 5*300, height = 6.5*300, res = 300)
        par(mar=c(0,0,0,0))
        ...
        png(file=file, type="cairo", units="in", width=5, height=5.5, pointsize=12, res=300)
        ...
        mtext(paste("Time period: ", proj.year.names[which(proj.years==proj.year)-1], sep=""), side=3, line=-3.5, adj=0.01, cex=0.8, font=1)
        ...
        # Legend
        par(plt = c(0.75, 0.999, 0.7, 1), xpd = TRUE, new = TRUE)
  ...
  scatterPlot <- function() {
    ...
    if(input$type==1) {
      ...
      if(input$plotbgc==1){
        ...
        order.increasing <- rev(order(unlist(as.vector(data[which(period$GCM==gcm.focal & period$proj.year==2081),increasing==T]))))
